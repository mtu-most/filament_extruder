			...
			rotate([90, 0, 0]) {
				for (j = [0, 1])
					mirror([0, 0, j])
						hull()
							for (i = [-1, 1])
								translate([i * qc_max_displacement / 2, 0, 6.6])
									cylinder(r1 = 2 * qc_max_displacement, r2 = qc_max_displacement / 2 + 0.1, h = 6);
...
module encoder_knob() {
	difference() {
		hull()
			for (i = [-1, 1])
				translate([0, 0, i * (d_encoder_knob / 2 - 2)])
					rotate_extrude(convexity = 10)
						translate([d_encoder_knob / 2 - 2, 0, 0])
							circle(r = 2);
		
		translate([0, 0, -d_encoder_knob / 2 - 1])
			difference() {
				cylinder(r = 3.1, h = 13.5);
...
module display_mount() {
	// interface mount
	l_2x16_display = 71.5; // measured 71.2
	w_2x16_display = 24.2; // measured 24.06
	h_2x16_display = 7; // measured 7
	cc_2x16_l_mounts = 75; // measured 75
	cc_2x16_w_mounts = 31; // measured 29
	l_2x16_overall = 81; // with the I2C backpack measured 81.5
	w_2x16_overall = 37; // with the I2C backpack measured 36.5
	h_2x16_overall = 18.5; // with the I2C backpack
	h_2x16_standoffs = 2.5; // because there is a truncated triangular prism at one end of the display, need standoffs
	lcd_2x16 = [
		l_2x16_display, 
		w_2x16_display, 
		h_2x16_display, 
	...
	offset_encoder = 	lcd[6] / 2 + 7.5;
	
		difference() {
			union() {
				hull() {
					for (i = [-1, 1], j = [-1, 1], k = [-1, 1])
						translate([i * (lcd[5] / 2), j * (lcd[6] / 2), k * (lcd[7] / 2 - t_bezel)])
							sphere(r = t_bezel);
						
					translate([0, offset_encoder, 0])
						for (i = [-1, 1])
							translate([0, 0, i * (lcd[7] / 2 - t_bezel)])
								rotate_extrude(convexity = 10)
									translate([d_encoder_knob / 2, 0, 0])
										circle(r = t_bezel);
				}
				
				hull() {
					translate([0, -lcd[6] / 2 - t_bezel - 4, -lcd[7] / 2])
						cylinder(r = 4, h = 4);
					
					translate([0, offset_encoder + t_bezel + d_encoder_knob / 2 + 4, -lcd[7] / 2])
						cylinder(r = 4, h = 4);
				}
			}
			// opening for display
			translate([0, 0, (lcd[7] - t_bezel) / 2 + layer_height])
				cube([lcd[0], lcd[1], t_bezel], center = true);

			// opening for board
			translate([0, 0, -t_bezel])
				difference() {
					cube([lcd[5], lcd[6], lcd[7]], center = true);
					
					// standoffs
					for (i = [-1, 1], j = [-1, 1])
						translate([i * lcd[3] / 2, j * lcd[4] / 2, (lcd[7] - lcd[8]) / 2]) {
							cylinder(r = 3.1, h = lcd[8], center = true);
							
							translate([0, 0, -(lcd[8] + layer_height) / 2])
								rotate([0, 0, j * -i * 45])
									cube([20, 8, layer_height], center = true);
						}
				}

			// lcd mount holes
			for (i = [-1, 1], j = [-1, 1])
				translate([i * lcd[3] / 2, j * lcd[4] / 2, lcd[7] / 2 - t_bezel - lcd[8]])
					cylinder(r = 1.3, h = t_bezel + lcd[8] - 1);
			
			// encoder mount
			translate([0, offset_encoder, 0]) {
				translate([0, 0, lcd[7] / 2 - t_bezel / 4 + layer_height])
					cylinder(r = 3.5, h = t_bezel / 2, center = true);
			
				translate([0, 0, -t_bezel / 2])
					cube([16, 16, lcd[7]], center = true);
			}
			
			translate([0, -lcd[6] / 2 - t_bezel - 4, -lcd[7] / 2 - 1])
				cylinder(r = d_M3_screw / 2, h = 6);
			
			translate([0, offset_encoder + t_bezel + d_encoder_knob / 2 + 4, -lcd[7] / 2 - 1])
				cylinder(r = d_M3_screw / 2, h = 6);

			// knockout for connector
			translate([lcd[5] / 2, 0, -lcd[7] / 2])
				cube([4 * t_bezel, lcd[6], 2 * t_bezel], center = true);
		}
}
...
//				metric_thread(diameter = d_hopper, pitch = pitch_hopper, length = h_threads, internal = true, n_starts = 1);
			...
			cylinder(r = d_hopper / 2, h = h_threads + 5);


				translate([0, 0, -40])
					cylinder(r = od_hopper_engagement / 2 + pad_threads + 15, h = 40);
